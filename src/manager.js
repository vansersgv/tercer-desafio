...
export class ProductManager {
	...
	async addProduct(product) {
		...
		if (!title || !description || !price || !thumbnail || !code || !stock) {
			console.log("El producto debe incluir los campos title, description, price, thumbnail, code y stock")

			return
		}

		this.products.find(element => element.code == product.code)
			? console.log("El código del producto ya existe") : this.products.push(product)
	...
	async getProductById(id) {
		...
		return this.products.find(product => product.id == id) ?? console.log('NO ENCONTRADO')
	}
